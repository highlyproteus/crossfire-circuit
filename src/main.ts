...
const voice=new VoiceChat(multiplayer);
let voiceWasPaused=false;
const voiceUI=new VoiceUI(voice,multiplayer,open=>{if(open){voiceWasPaused=sim.paused;pause(true);el('pause-panel').classList.add('hidden');}else pause(voiceWasPaused);});
...
function syncSound(){el('sound').textContent=gameAudio.enabled?'GAME SOUND ON':'GAME SOUND OFF';checkbox('sound-enabled').checked=gameAudio.enabled;}
...
addEventListener('keydown',e=>{
  ...
  if(e.code==='KeyM'&&!e.repeat&&multiplayer.room){e.preventDefault();void voice.toggleMicrophone();return;}
...
if(new URLSearchParams(location.search).has('qa')){
  (window as any).gameDebug={snapshot:()=>sim.snapshot(),multiplayer,voice,sim,view,touch,readInput,frame,center,jumps,checkpoints,COURSE_LENGTH,closest,driveAI,coverPositions,obstacles,routes,boosts,barrelSpots,gameAudio,
