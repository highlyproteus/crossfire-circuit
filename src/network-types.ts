...
export type WorldState={roomId:string;leader:string;stage:RoundPhase;remaining:number;elapsed:number;round:number;reason:string;marksman:string;players:PlayerState[];barrels:{id:number;p:Vec;q:{x:number;y:number;z:number;w:number};active:boolean;explosive:boolean}[];effects:Effect[];rockets:Rocket[];tick:number};
