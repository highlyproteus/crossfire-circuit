...
const config={company:'Test organization',awsAccountId:'111122223333',awsProfile:'crossfire-test',region:'us-east-2'};
function run(account,args,configText=JSON.stringify(config)){
 const dir=mkdtempSync(join(tmpdir(),'crossfire-ownership-'));
 try{
  const isolatedWrapper=join(dir,'aws.mjs');
  copyFileSync(wrapper,isolatedWrapper);
  if(configText!==null)writeFileSync(join(dir,'production.json'),configText);
  ...
  const result=spawnSync(process.execPath,[isolatedWrapper,...args],{encoding:'utf8',env:{...process.env,PATH:dir+':'+process.env.PATH,AWS_ACCESS_KEY_ID:'test-ambient-credential'}});
...
test('verified account dispatches with explicit profile and no ambient credentials',()=>{
...
test('missing, malformed, or incomplete local configuration never invokes AWS',()=>{
 const invalid=[null,'{',JSON.stringify({}),JSON.stringify({...config,awsAccountId:111122223333}),JSON.stringify({...config,awsAccountId:'123'}),JSON.stringify({...config,awsProfile:''}),JSON.stringify({...config,region:''}),JSON.stringify({...config,company:''}),readFileSync(new URL('./production.example.json',import.meta.url),'utf8')];
 for(const configText of invalid){
  const r=run(config.awsAccountId,['ec2','describe-instances'],configText);
  assert.equal(r.status,2);assert.equal(r.invoked.length,0);assert.match(r.stderr,/Missing or invalid local/);
 }
});
