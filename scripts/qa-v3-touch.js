async(page)=>{
 ...
 await tap(page.getByRole('button',{name:'ADS',exact:true}));
 ...
 await tap(page.getByRole('button',{name:'RELOAD',exact:true}));await page.evaluate(()=>{window.touchQA.reload=window.gameDebug.sim.reloadTime>0;window.gameDebug.advance(160,{throttle:0,steer:0,brake:false});});await tap(page.getByRole('button',{name:'2 · ROCKETS'}));await page.evaluate(()=>{window.touchQA.weapon=window.gameDebug.sim.attackType;});await page.screenshot({path:'mobile-marksman.png'});
}
