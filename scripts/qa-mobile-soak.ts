...
const updated=new Map<string,number>();let drops=0,reconnects=0;
...
try{
 for(let i=0;i<count;i++){const client=new Client(endpoint);const room=i?await client.joinById(rooms[0].roomId,{name:'QA Driver '+i}):await client.create('circuit',{name:'QA Leader'});rooms.push(room);room.onMessage('world',(s:WorldState)=>{states.set(room.sessionId,s);updated.set(room.sessionId,Date.now());if(i===0){summary.frames++;const now=Date.now();if(last)summary.maxGap=Math.max(summary.maxGap,now-last);last=now;if(!summary.stages.includes(s.stage))summary.stages.push(s.stage);if(s.stage==='racing'){summary.marksmen=s.players.filter(p=>p.role==='marksman').length;if(!firstTime){firstTime=now;firstTick=s.tick;}else summary.tickRate=(s.tick-firstTick)/(now-firstTime)*1000;}}});room.onDrop(()=>drops++);room.onReconnect(()=>reconnects++);room.onMessage('notice',()=>{});room.onMessage('pong',()=>{});room.onError((_c,m)=>summary.errors.push(m??'error'));}
 ...
 rooms[0].send('start');await sleep(125000);clearInterval(timer);
 console.log(JSON.stringify({...summary,room:rooms[0].roomId,drops,reconnects,staleClients:[...updated.values()].filter(t=>Date.now()-t>3000).length,uniquePlayers:states.get(rooms[0].sessionId)?.players.length,clientViews:states.size},null,2));
 if(!summary.stages.includes('countdown')||summary.marksmen!==1||states.size!==count||summary.tickRate<55||summary.errors.length||[...updated.values()].some(t=>Date.now()-t>3000)||drops>reconnects)process.exitCode=1;
