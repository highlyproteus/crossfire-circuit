...
Production AWS ownership and host identity are recorded in `deploy/production.json`. Use `node deploy/aws.mjs` for AWS operations: it checks the signed-in account before dispatching a command. Stop on an ownership mismatch instead of selecting another available account or host. Use the existing Proteus Vercel and Crossfire Convex projects documented in `docs/hosting.md`.
